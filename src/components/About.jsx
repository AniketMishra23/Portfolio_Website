        ...
        <div className="grid md:grid-cols-2 gap-12 items-center mt-12">
          ...
          >
            <div className="relative">
              <div className="w-52 h-52 rounded-2xl bg-gradient-to-br from-[#00f5ff22] to-[#bf00ff22] border border-[#00f5ff44] flex items-center justify-center shadow-[0_0_40px_#00f5ff22]">
                <span className="text-7xl font-bold neon-text font-mono">AM</span>
              </div>
              <div className="absolute -bottom-3 -right-3 w-52 h-52 rounded-2xl border border-[#bf00ff44] -z-10" />
            </div>
          </motion.div>
...
export function SectionHeader({ label, title }) {
  return (
    <div className="flex items-center gap-4">
      <span className="font-mono text-[#00f5ff] text-sm opacity-50">{label} /</span>
      <h2 className="text-3xl md:text-4xl font-bold text-white">{title}</h2>
      <div className="flex-1 h-px bg-gradient-to-r from-[#00f5ff33] to-transparent" />
    </div>
  );
}
